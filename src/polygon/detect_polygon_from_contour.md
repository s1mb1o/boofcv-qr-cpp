...
2. **Per-blob pre-filtering** (drops cheaply-detected non-polygons before the expensive corner-finder runs):
   ...
   **Perf note (2026-05-11 target 7):** `ContourEdgeIntensity::process()` bounds-checks each tangent-normal sample before interpolation. The hot path therefore uses an in-bounds bilinear helper (`sampleInside`) that skips the private `sample()` helper's EXTENDED-border clamp and uses row-pointer image reads instead of `cv::Mat::at()`. The clamped helper remains available for the border-policy semantics; it clamps first, then forwards through the same bilinear core. Positive-coordinate truncation is equivalent to `floor()` for these proven in-bounds samples, so interpolation weights and edge averages are unchanged.
