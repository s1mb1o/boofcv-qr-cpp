...
- **Pure-point DLT null-space solve** — parity-load-bearing since cycle 3. It now uses a fixed 9×9 eigensolve instead of repeated `cv::SVD::solveZ`; reverting to `cv::findHomography` would re-open the -0.08pp parity residual cycle 3 closed. The mixed line-DLT path still uses the SVD right-singular-vector objective because it is more precise than the tested normal-equation replacement on the rotated line fixture, but its fixed 14×9 system now uses compact `cv::SVD::compute` without the unused left singular basis.
