...
- Homography math only: `cv::getPerspectiveTransform` (4-point matrix), `cv::perspectiveTransform` (transform individual points), `cv::invert` (matrix inverse). Do **not** use `cv::warpPerspective` or `cv::remap` to rectify a QR for bit sampling — BoofCV samples the source binary directly at homography-mapped grid coordinates, and OpenCV's interpolation / rounding / border-replication choices would change sampled bits. `cv::warpPerspective` and `cv::remap` are permitted only outside the QR sampling path (e.g. visualization or debug renders). For the **N-point (N>4) DLT** case, do **not** use `cv::findHomography(..., 0)`: despite the `method=0` flag, OpenCV runs an iterative Levenberg–Marquardt refinement after the initial DLT for `npoints > 4` (`cv::LMSolverImpl::run` → `cv::solve` → `cv::JacobiImpl_<double>` chain, profile-confirmed). BoofCV's `GenerateHomographyLinear` → `HomographyDirectLinearTransform` runs only DLT, no LM. Build the 2N×9 design matrix explicitly and solve with `cv::SVD::solveZ` to mirror BoofCV's `SolveNullSpaceSvd_DDRM` exactly (see [src/sampler/qr_code_binary_grid_to_pixel.md](src/sampler/qr_code_binary_grid_to_pixel.md) "Why this approach" for the row layout).
